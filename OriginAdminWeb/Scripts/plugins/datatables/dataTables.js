...
//选中行
    ...
    CommonTable.prototype.exportByTemplate=function(template,tableName,sheetConfig){
        ...
        ajaxPost(basePath+"/query/exportDataByTemplate",{reqParam:JSON.stringify(reqParam),tableName:tableName,template:template},function(data) {
            if(data.success) {
    ...
    CommonTable.prototype.exportMultiData=function (tableName,queryConditions) {
        ajaxPost(basePath+"/query/exportData",{reqObjs:JSON.stringify(queryConditions),tableName:tableName},function(data) {
            if(data.success) {
                window.location.href = basePath+'/query/downExport?tempfile='+data.data+"&tableName="+encodeURIComponent(tableName);
            }
        })
